...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'Inbox'
        db.create_table('threads_inbox', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(related_name='inbox_user', to=orm['auth.User'])),
        ))
        db.send_create_signal('threads', ['Inbox'])

        # Adding M2M table for field threads on 'Inbox'
        db.create_table('threads_inbox_threads', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('inbox', models.ForeignKey(orm['threads.inbox'], null=False)),
            ('thread', models.ForeignKey(orm['threads.thread'], null=False))
        ))
        db.create_unique('threads_inbox_threads', ['inbox_id', 'thread_id'])


    def backwards(self, orm):
        # Deleting model 'Inbox'
        db.delete_table('threads_inbox')

        # Removing M2M table for field threads on 'Inbox'
        db.delete_table('threads_inbox_threads')
